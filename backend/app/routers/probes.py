...
def submit_probe_responses(
    ...
    if scored_any:
        branch_core_memory = merge_questionnaire_scores_into_core_memory(
            target_user.core_memory,
            target_user.mbti_type,
            big_five_scores if big_five_scores is not None else target_user.big_five_scores,
            schwartz_values if schwartz_values is not None else target_user.schwartz_values,
        )
        if branch_id == DEFAULT_BRANCH_ID:
