...
modes, chat with other human participants in branch-aware 1:1 and human-only
group rooms, run isolated Agent-only group/debate simulations, upload/search
memories, import group-chat history, trigger sleep-style consolidation, inspect
relationship/memory state, fork timelines into counterfactual branches, collect
blind-test authenticity ratings, and export JSONL research data.
...
- Loop now has persistent social chat. Human-to-human and human-only group messages are stored in `ChatLog` with `SessionType.HUMAN_TO_HUMAN` / `GROUP_SHARED`, branch visibility uses fork lineage windows, and realtime delivery uses `/api/social/events` SSE plus optional Redis fanout.
- Group rooms enforce "Boundary 1" species isolation: `HUMAN_ONLY` groups contain `USER` members only, while `AGENT_ONLY` groups contain `AGENT` members only. Do not mix these in CRUD, imports, or UI shortcuts.
- The `/sandbox` surface is admin-only and separate from `/lab`: it runs supervised multi-Agent debates and manual Agent-only group turns without exposing those controls to ordinary participants.
...
- Chat turns are isolated by `branch_id`, `session_id`, `topic`, and `session_type`; do not accidentally merge private Agent chat, human 1:1 chat, and group rooms when loading history, routing context, drift checks, social unread counts, or exports.
...
- `Group`: N-to-N room metadata for `HUMAN_ONLY` and `AGENT_ONLY` rooms, with optional owner and topic
- `GroupMember`: member rows keyed by `entity_type` (`USER` or `AGENT`) and `entity_id`; CRUD enforces species isolation
- `GroupSummary`: branch-scoped rolling compressed context for long group-room histories
- `ChatLog`: private sync turns, human-to-human messages, and shared group messages with sender/receiver/group metadata, branch id, session id, topic, experiment mode, session type, read/extraction flags, and second-precision timestamp
...
- Human social chat is a persisted research signal, not disposable UI state. 1:1 messages append `HUMAN_MESSAGE_RECEIVED`; human group messages append `GROUP_MESSAGE_RECEIVED`; both are branch-aware `ChatLog` rows and can trigger bypass memory extraction after a sliding window of user-authored messages.
- Agent-only group turns and supervised debates are admin/machine simulation paths. `speaker_manager.py` chooses one Agent speaker at a time for `AGENT_ONLY` rooms and stores the turn as group chat; `debate_graph.py` uses a moderator-style LangGraph loop and writes debate messages/conclusion events.
- Long group-room context is protected by `rolling_summary.py`: only the latest bounded raw messages stay in prompt context, while ejected messages are compressed into branch-scoped `GroupSummary` rows.
...
- Backend list endpoints must continue to enforce bounded `limit` values: `/api/plaza/events`, `/api/posts`, `/api/agents/{agent_id}/chat`, `/api/agents/{agent_id}/events`, `/api/social/contacts`, `/api/social/messages/{contact_id}`, `/api/social/groups`, and `/api/social/groups/{group_id}/messages` are intentionally paginated to protect long-running experiments.
...
```text
Human 1:1 / human group / Agent-only group simulation
  -> ChatLog rows with session_type + branch_id + sender/receiver/group metadata
  -> EventLog appends HUMAN_MESSAGE_RECEIVED or GROUP_MESSAGE_RECEIVED
  -> SSE/Redis fanout delivers realtime notifications when relevant
  -> bypass memory watcher or rolling summary compresses high-volume social context
```
...
GET  /api/agents/directory                          [admin bearer]
...
POST /api/simulate/debate                           [admin bearer or machine key]
POST /api/simulate/groups/{group_id}/tick           [admin bearer or machine key]
...
- Social realtime endpoints authenticate bearer tokens through a `token` query parameter because EventSource/WebSocket clients cannot reliably send custom `Authorization` headers. Keep token lifetime and transport exposure in mind.
...
- `Base.metadata.create_all()` creates new tables. Startup also ensures `agents.is_npc`, `probe_responses.branch_id`, chat social/group columns (`session_type`, `sender_user_id`, `receiver_user_id`, `group_id`, `is_memory_extracted`, `is_read`), `groups.owner_id`, branch-scoped group summaries, social chat indexes, `users.is_admin`, the configured admin user, the `vector` extension, the pgvector-backed `rag_documents` table and indexes, and Postgres append-only triggers for `event_logs`.
...
- Memory upload/search and sleep consolidation RAG writes are stored in Postgres `rag_documents`, not under a local vector-store directory. Upload/search payloads now accept `branch_id` and validate that the branch exists.
...
- `create_human_to_human_chat_log()` appends `HUMAN_MESSAGE_RECEIVED`; `create_group_message_log()` appends `GROUP_MESSAGE_RECEIVED`. These reuse `ChatLog` with `session_type`, so downstream readers must filter by session type when they only want private Agent chat.
- Social list readers use `get_branch_read_windows()` and `branch_window_filter()`, so forked branches can see appropriate main-line history without flattening all branch messages together.
- `/api/social/events` is the frontend's primary realtime path. `frontend/dev-server.mjs` special-cases this SSE stream and proxies it to `BACKEND_ORIGIN` (default `http://127.0.0.1:8001`) because ordinary Next dev rewrites are not enough for reliable long-lived event streams.
- `social_realtime.py` delivers notifications to local SSE queues and, when `LOOP_REDIS_URL` is set, publishes across workers with recent-event dedupe. Without Redis, realtime works only within the current backend worker.
- Human 1:1 and human group messages can trigger bypass memory watchers once 20 unprocessed user-authored messages accumulate. They merge safe style/persona insights into `core_memory` on `main`; non-main insights become branch `CORE_MEMORY_UPDATED` events.
- `Group` CRUD enforces Boundary 1. `HUMAN_ONLY` groups reject Agent members, and `AGENT_ONLY` groups reject User members; keep this invariant even in admin tooling.
- `POST /api/simulate/groups/{group_id}/tick` advances exactly one Agent speaker turn for an `AGENT_ONLY` group, with process-local and Postgres advisory locks to avoid duplicate turns.
- `POST /api/simulate/debate` runs the supervised LangGraph debate path. Debate turns are appended as `GROUP_MESSAGE_RECEIVED` events, and the final moderator report is appended as `DEBATE_CONCLUDED`; this path does not currently create `ChatLog` rows for every debate turn.
...
- User-facing memory upload/search endpoints now accept `branch_id`. They still store/retrieve through pgvector metadata and Git-style branch filtering rather than a local vector directory.
...
- `backend/app/services/rolling_summary.py`: branch-aware rolling summary for long group rooms; protects prompts by keeping only a bounded raw-message window.
- `backend/app/services/speaker_manager.py`: one-turn Agent speaker selection and generation for `AGENT_ONLY` rooms, with advisory locking.
- `backend/app/services/debate_graph.py`: supervised multi-Agent debate graph, moderator routing, memory retrieval, and debate conclusion event writing.
- `backend/app/services/social_realtime.py`: SSE social notification hub with optional Redis pub/sub coordination.
- `backend/app/services/ws_manager.py`: lightweight WebSocket connection manager retained for `/api/ws/social`.
...
- Agent-only group turns use `DEEPSEEK_CHAT_MODEL` and normal chat thinking settings. Supervised debates use `LOOP_DEBATE_MODEL`, `LOOP_DEBATE_LLM_TIMEOUT_SECONDS`, and `LOOP_DEBATE_MAX_TOKENS`, falling back to `DEEPSEEK_CHAT_MODEL` when the debate model is not set.
...
- A custom `frontend/dev-server.mjs` wrapper around Next dev, used so `/api/social/events` can be proxied as a long-lived SSE stream
...
- `frontend/next.config.mjs` currently rewrites `/api/:path*` to the hardcoded backend target `http://127.0.0.1:8001/api/:path*`. Keep it aligned if the backend port changes.
- `frontend/dev-server.mjs` separately proxies `/api/social/events` to `BACKEND_ORIGIN` (default `http://127.0.0.1:8001`) so EventSource realtime chat stays open during development.
...
Restart the Next.js dev server after changing `frontend/.env.local`, `frontend/dev-server.mjs`, or `frontend/next.config.mjs`; `NEXT_PUBLIC_*` variables and rewrites are bundled at dev-server startup/build time.
...
- `/social`: branch-aware human participant social space with paginated contact directory, persisted 1:1 chat, human group rooms, unread/read state, and SSE realtime notifications.
- `/sandbox`: admin-only simulation workbench for supervised multi-Agent debates, Boundary-1 group creation/member management, and manual Agent-only group ticks.
...
- Social page uses `GET /api/social/contacts`, paginated `GET /api/social/messages/{contact_id}`, `POST /api/social/messages`, and `POST /api/social/messages/{contact_id}/read` for 1:1 chat. It uses `GET/POST /api/social/groups` plus paginated group-message endpoints for human group rooms.
- Social realtime is received through `EventSource` on `/api/social/events?token=...`; pushed payloads are merged with optimistic pending messages so REST persistence remains the source of truth.
- Sandbox uses `GET /api/agents/directory`, `POST /api/simulate/debate`, `POST /api/groups`, `POST /api/groups/{group_id}/members`, and `POST /api/simulate/groups/{group_id}/tick`.
...
- Memory, chat, plaza, probes, counterfactuals, and social pages use `BranchSelector`, but participant branch switching is gated by `SystemSetting.allow_user_branch_switch`. Sandbox is admin-only and always exposes branch selection.
...
`frontend/src/app/social/page.tsx` is the branch-aware human social space.

- Loads branch settings like plaza/chat; admins can switch branches, ordinary users follow `SystemSetting` exposure.
- Uses `H2HChatPanel` for persisted human-to-human chat with paginated contact search, unread counts, read receipts, optimistic sends, and REST history reloads.
- Uses `HumanGroupPanel` for `HUMAN_ONLY` group rooms, paginated group lists, group creation from selected contacts, and bounded group-message history.
- Receives realtime notifications through `EventSource` on `/api/social/events?token=...`; the dev server proxies this path specially for stable SSE.
- All messages remain persisted in `ChatLog`; realtime delivery is a notification path, not the durable source of truth.

### `/sandbox`

`frontend/src/app/sandbox/page.tsx` is the admin-only simulation workbench.

- Redirects non-admin sessions away from the page.
- Lets admins choose a branch, run a supervised debate through `POST /api/simulate/debate`, and inspect the moderator final report.
- Provides a group simulation panel for creating `HUMAN_ONLY` or `AGENT_ONLY` rooms, adding members, and manually ticking one `AGENT_ONLY` speaker turn.
- Uses `/api/agents/directory` for Agent choices and the generic `/api/groups` endpoints for Boundary-1 group setup.
...
- Uploads long-form memory chunks with a selected `branch_id`.
- Runs semantic search against pgvector memory with branch-aware retrieval.
- Reads global branch settings; branch selection affects upload/search payloads and working-memory/core-memory reconstruction.
...
- `HUMAN_MESSAGE_RECEIVED`: a human-to-human social message was stored.
- `GROUP_MESSAGE_RECEIVED`: a human group, Agent-only group turn, or debate-style group message was recorded.
- `DEBATE_CONCLUDED`: supervised debate graph finished and wrote its moderator report.
...
14. Open `/social`, send a human 1:1 message and create a human group message; verify the message persists after reload.
15. As admin, open `/sandbox`, run a small debate or manually tick one Agent-only group.
16. Open `/probes`, submit the current IPIP/PVQ probe set, and confirm the Agent profile still loads.
17. Open `/counterfactuals`, submit one life-decision anchor, and confirm memory diagnostics show the updated core memory.
18. Open `/evaluations/{agent_id}` in a fresh browser context and submit a blind-test rating.
19. Open `/memory`, upload/search memory, trigger sleep consolidation, and refresh diagnostics.
20. Open `/time-machine`, load events, fork a branch from an event, and verify branch selectors include the new branch.
21. In `/lab`, export chatlogs or feedbacks as JSONL with the admin bearer session.
...
Restart `npm run dev` after changing this file. Keep `NEXT_PUBLIC_API_BASE_URL` empty so the browser calls the tunneled frontend origin, not FastAPI directly. The current `next.config.mjs` hardcodes the REST rewrite target, while `dev-server.mjs` uses `BACKEND_ORIGIN` for the SSE path; keep both aligned with the backend port.
...
- Browser `127.0.0.1` means the user's local computer. Use SSH local forwarding from the personal computer, keep `NEXT_PUBLIC_API_BASE_URL` empty for same-origin frontend calls, and keep `frontend/next.config.mjs` plus `frontend/dev-server.mjs` aligned with the FastAPI loopback port.
